...
bool PlayVideo::playStart(videospec_t video)
{
    ...
    else if ((video.dvd_filename.length() > 2) && (video.dvd_filename.at(0) == YT_STREAM_MARK))
    {
        dvd_filename = video.dvd_filename.substr(1); // remove prepended character
        is_yt = true; // tell omxplayer this is an audio stream file
    }
    else if((video.dvd_filename.length() > 2) && (video.dvd_filename.at(0) == AUDIO_STREAM_MARK))
    {
        dvd_filename = video.dvd_filename.substr(1); // remove prepended character
        is_audio = true; // tell omxplayer this is an audio stream file
    }
    else
    {
    ...
    if(is_yt == true || is_audio == true)
    {
       playString.clear();
       std::ifstream ifs(VFN);
        std::string urlString;
        urlString.assign( (std::istreambuf_iterator<char>(ifs) ),
                (std::istreambuf_iterator<char>()    ) );
                if(is_yt == true)
                playString = PPPath + " " + "`youtube-dl -g -f mp4 " + urlString + "`";
                else
                playString = PPPath + " " + urlString;
        cout << "PV: " << playString << endl;
    }
