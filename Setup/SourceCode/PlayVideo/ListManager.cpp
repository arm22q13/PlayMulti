...
void ListManager::initialize(string input_list_filename)
{
    ...
    while (getline(listfile, line))
    {
       ...
       // If first character of file name is @ sign, that file will set to autorepeat (loop).
		...
		// If first character of file name is + sign, that file is marked as an audio stream.
		if (fn.at(0) == AUDIO_STREAM_MARK)
        {
            cout << "LM: This is an audio stream file." << endl;
			videos[current_file_pointer].file_type = FILE_TYPE_AUDIO_STREAM;
        }


		//Additional file types can be added here.

		else if (fn.at(0) == YT_STREAM_MARK)
		{
			cout << "LM: This is a youtube stream file." << endl;
			videos[current_file_pointer].file_type = FILE_TYPE_YT_STREAM;
		}

		// If no file type identifier marks are detected, the file is marked as a video.
		else
		{
			cout << "LM: This is a video file." << endl;
			videos[current_file_pointer].file_type = FILE_TYPE_VIDEO;
		}
